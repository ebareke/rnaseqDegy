...
for (gmt_file in gmt_files) {
  ...
  ## generate GSEA plot
  #enrich_dot_plot <- dotplot(enrich_result, showCategory = 15) + ggtitle("Over-representation analysis : Day 5")
  #gsea_dot_plot <- dotplotGsea(data = gsea_result, topn = 15)
  gsea_dot_plot <- dotplot(gsea_result, showCategory = 10, split = ".sign") + facet_grid(.~.sign) + ggtitle("GeneSet Enrichment Analysis : Day 5") + theme_bw() +
