...
`rnaseqDegy` is an R package providing a **single, fully automated
pipeline** for bulk RNA‑seq analysis — covering **QC**, **differential
expression**, **GO ORA**, and **GSEA** using **user‑supplied GMT
files**, for both **human and mouse** datasets.

The package is designed for **research labs**, **HPC clusters**, and
**automated workflows**, offering clean, reproducible, publication‑ready
outputs.
...
This package requires **Bioconductor dependencies**. Install everything
cleanly with:
...
This will generate: - QC plots - DE tables + volcano - ORA (BP/MF/CC) -
GSEA (for each GMT)
